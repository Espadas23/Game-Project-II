...
public class MapController : MonoBehaviour
{
    ...
    public Vector2 mapOrigin;         // левый нижний угол карты в мировых координатах
    public float mapWidth;            // ширина карты в мировых единицах
    public float mapHeight;           // высота карты в мировых единицах
    ...
    void Start()
    {
        ...
        ClearFog(); // очищаем туман при старте
    ...
    void HandleMapToggle()
    {
        if (Keyboard.current != null && Keyboard.current.mKey.wasPressedThisFrame)
        {
            ...
            if (mapVisible)
                ClearFog(); // очищаем туман при открытии карты
        }
    }
    ...
    void UpdateMapAlpha()
    {
        ...
        mapMaterial.SetFloat(shaderAlphaProp, Mathf.MoveTowards(currentAlpha, targetAlpha, Time.deltaTime * mapFadeSpeed));
    }

    void UpdateFogTexture()
    {
        if (!mapVisible || fogTexture == null || player == null)
            return;

        // координаты игрока относительно карты
        float u = Mathf.InverseLerp(mapOrigin.x, mapOrigin.x + mapWidth, player.position.x);
        float v = Mathf.InverseLerp(mapOrigin.y, mapOrigin.y + mapHeight, player.position.y);

        int px = Mathf.RoundToInt(u * fogTextureSize);
        int py = Mathf.RoundToInt(v * fogTextureSize);
        ...
        for (int y = -revealRadius; y <= revealRadius; y++)
        {
            ...
            for (int x = -revealRadius; x <= revealRadius; x++)
            {
                ...
                if (x * x + y * y <= revealRadius * revealRadius)
                    fogPixels[ty * fogTextureSize + tx] = Color.white;
            }
        }
