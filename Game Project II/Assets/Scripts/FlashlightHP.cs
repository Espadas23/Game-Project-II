...
public class FlashlightHP : MonoBehaviour
{
    ...
    [Header("Flashlight HP Penalty")]
    public float flashlightPenaltyOnScreamer = 5f; // сколько секунд отнимается при скримере
    ...
    public void OnCrystalCollected()
    {
        ...
        if (flashlightScript != null)
        {
            flashlightScript.RestoreRadius(maxRadius);
    ...
    public void ApplyScreamerPenalty(float seconds)
    {
        timerFlashlight += seconds; 
        if (timerFlashlight > shrinkDuration)
            timerFlashlight = shrinkDuration;
    }
